      ...
      label: (
        <div className={styleLess.setting_menu}>
          <Row className={styleLess.setting_menu__row}>
            <Col md={12}>
              <EllipsisText
                ...
                text={intl.formatMessage({ id: 'sale.order.management.column.setting' })}
              />
            </Col>
            <Col md={12}>
              <EllipsisText
                className={styleLess.setting_menu__row__total_selected}
                text={`${columnSelected.length} ${intl.formatMessage({ id: 'sale.order.management.columns.selected' })}`}
              />
            </Col>
          </Row>
          <Row>
          ...
          <ButtonWrapper
            ...
            variant="primary"
            text={intl.formatMessage({ id: 'sale.order.management.columns.apply' })}
            onClick={handleSelectColumns}
          />
          <Row>
            <Col md={24} className={styleLess.setting_menu__row__warning}>
              {intl.formatMessage(
                { id: 'sale.order.management.columns.warning' },
  ...
  return (
    <Row>
      <Col lg={6} sm={12} xs={24} className={styleLess.setting}>
        <Tooltip
          placement="topLeft"
          title={intl.formatMessage({ id: 'sale.order.management.column.setting' })}
        >
          <DropdownWrapper
            menu={{ items }}
            open={visible}
