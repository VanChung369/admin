...
import { TAG_MANAGEMENT_FIELD } from '@/pages/Tag/constants';

const { PAGE, LIMIT } = TAG_MANAGEMENT_FIELD;
const { DEFAULT_PAGE, DEFAULT_PAGE_SIZE } = LENGTH_CONSTANTS;
const { ORDER, FIELD } = ORDERS;

const ListTable = ({ total, data, loading, onSetParams, params, emtpyText }: ListTableProps) => {
  const intl = useIntl();
  ...
  const selectedColumns = columns(intl, page, limit);
