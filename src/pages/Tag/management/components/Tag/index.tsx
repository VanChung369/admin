...
import { useGetListTags } from '@/pages/Tag/hooks';
...
const TagList = ({ params, onSubmit }: tagProps) => {
  const intl = useIntl();
  const { data, loading } = useGetListTags(params);
  ...
  return (
    <Fragment>
      ...
      <TypographyWrapper
        className={styleLess.tag_total}
        typeTypography={TYPE_TYPOGRAPHY.TEXT_HELP}
        label={intl.formatMessage({ id: 'tag.order.management.total' })}
      >
